defmodule Photor.Imports.ImportSessionTest do
  ...
  describe "import session" do
    ...
    test "initializes with correct state", %{import: import} do
      ...
      assert state.files_found == []
      assert state.files_to_import == []
      assert state.files_skipped == []
      assert state.files_imported == []
      assert state.files_with_errors == []
    ...
    test "processes files found event", %{import: import} do
      ...
      assert state.files_found == files
      assert state.files_to_import == files
    ...
    test "processes file skipped event", %{import: import} do
      ...
      state = ImportSession.get_state(import.id)
      assert length(state.files_skipped) == 1
      [skipped] = state.files_skipped
    ...
    test "processes file importing and imported events", %{import: import} do
      ...
      state = ImportSession.get_state(import.id)
      assert state.current_file == nil
      assert length(state.files_imported) == 1
      [imported] = state.files_imported
    ...
    test "processes import finished event", %{import: import} do
      ...
      assert length(state.files_imported) == 2
